...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    ...
    // ?review=<orderId> — auto-open review modal (from reminder email)
    // Handled below once orders load
  }, [searchParams])

  // Auto-open review modal when orders load + URL has ?review=<id>
  useEffect(() => {
  ...
  // Load conversation list when messages tab opens
  useEffect(() => {
  ...
  // Load thread when activeOrderId changes
  useEffect(() => {
  ...
  // Real-time subscription per order thread
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Only add if not already in state (prevents double on sender side)
  ...
  async function loadConversations() {
    ...
    // Get all messages involving this user, join order info
    ...
    // Group by order_id — most recent message per order
    ...
    // For threads with order_id, enrich with order info
    const orderIds = threads.filter(t => t.order_id).map(t => t.order_id)
    let ordersMap: Record<string, any> = {}
    if (orderIds.length > 0) {
      const { data: ordersData } = await supabase
        .from('orders').select('id, event_type, event_date, status').in('id', orderIds)
      for (const o of ordersData || []) ordersMap[o.id] = o
    }

    setConversations(threads.map(t => ({
      ...t,
      order_info: t.order_id ? ordersMap[t.order_id] : null,
      thread_key: t.order_id || ('general-' + t.baker_id),
    })))

    // Auto-open from URL param or first conversation
    const paramOrderId = searchParams.get('order')
    if (paramOrderId && !activeOrderId) {
      setActiveOrderId(paramOrderId)
    } else if (!activeOrderId && threads.length > 0) {
      setActiveOrderId(threads[0].order_id || null)
    }
  }
  ...
  async function sendMessage() {
    ...
    const { data: bakerUser } = await supabase
      .from('bakers').select('user_id').eq('id', activeConvo.baker.id).maybeSingle()
    ...
    // Reload thread to get the new message (realtime handles live updates)
    loadThread(activeOrderId)
    loadConversations()
  }

  function openThread(orderId: string) {
  ...
  async function submitReview() {
    ...
    await supabase.from('reviews').insert({
      ...
      baker_id: reviewOrder.baker_id,
      customer_email: customer.email,
      customer_name: customer.full_name || '',
      event_type: reviewOrder.event_type,
      rating: reviewRating,
      comment: reviewComment,
    })
    // Update baker's cached avg_rating + review_count
    const { data: allReviews } = await supabase
      .from('reviews').select('rating').eq('baker_id', reviewOrder.baker_id)
  ...
  // Returns true if this order is eligible for the 48hr delayed review nudge
  ...
  function threadLabel(conv: any) {
    if (conv.order_info) return conv.order_info.event_type + ' · ' + conv.order_info.event_date
  ...
  async function addImportantDate() {
    ...
    const { data } = await supabase
      ...
      .select().single()
    if (data) {
      setImportantDates(prev => [...prev, data].sort((a, b) => a.month - b.month || a.day - b.day))
      setNewDateLabel('')
      setNewDateMonth('')
      setNewDateDay('')
    }
  ...
  const upcomingOrder = orders
    .filter(o =>
      ...
      getDaysUntil(o.event_date) >= 0 &&
      getDaysUntil(o.event_date) <= 30
    )
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: '#f5f0eb' }}>
      ...
      {reviewOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <p className="text-center text-xs mb-4 font-medium" style={{ color: '#5c3d2e' }}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={submitReview} disabled={submittingReview}
                ...
                style={{ backgroundColor: '#2d1a0e', opacity: submittingReview ? 0.7 : 1 }}>
      ...
      {/* Success toast */}
      {showSuccessToast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3.5 rounded-2xl shadow-lg flex items-center gap-3"
          style={{ backgroundColor: '#2d1a0e', color: 'white', minWidth: '300px' }}>
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: '#5c3d2e' }}>
              {orders.length === 0 ? "Ready to find your perfect baker?"
                : 'You have ' + pending.length + ' pending and ' + confirmed.length + ' confirmed order' + (confirmed.length !== 1 ? 's' : '') + '.'}
            </p>
          </div>
          <Link href="/bakers" className="px-5 py-2.5 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: '#2d1a0e' }}>
            + New Order
          </Link>
        </div>
          ...
          <div className="mb-6 rounded-2xl p-6 flex items-center justify-between"
            ...
            <div className="flex items-center gap-4">
              {upcomingOrder.bakers?.profile_photo_url ? (
                <img src={upcomingOrder.bakers.profile_photo_url} alt={upcomingOrder.bakers.business_name}
                ...
                <div className="w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center"
                  ...
                  <span className="text-white font-bold text-lg">
                    {upcomingOrder.bakers?.business_name?.[0] || 'B'}
                  </span>
                </div>
              )}
              <div>
                <p className="text-white font-bold text-lg">
                  {getDaysUntil(upcomingOrder.event_date) === 0
                    ? 'Your ' + upcomingOrder.event_type + ' order is today!'
                    : getDaysUntil(upcomingOrder.event_date) === 1
                    ? 'Your ' + upcomingOrder.event_type + ' order is tomorrow!'
                    : getDaysUntil(upcomingOrder.event_date) <= 7
                    ? 'Your ' + upcomingOrder.event_type + ' order is almost here!'
          ...
          <div className="flex flex-col gap-4">
            {orders.length === 0 ? (
              <div className="bg-white rounded-2xl p-16 shadow-sm text-center">
                ...
                <Link href="/bakers" className="px-6 py-3 rounded-xl text-white font-semibold text-sm inline-block" style={{ backgroundColor: '#2d1a0e' }}>
                  Browse Bakers
                </Link>
              </div>
            ) : (
              orders.map(order => {
                ...
                return (
                  <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm">
                        ...
                        {order.fulfillment_type === 'pickup' ? (
                          <div>
                            ...
                            {order.bakers?.pickup_address ? (
                              <a href={'https://maps.google.com/?q=' + encodeURIComponent(order.bakers.pickup_address)}
                                target="_blank" rel="noopener noreferrer"
                                className="text-sm font-semibold underline" style={{ color: '#2d1a0e' }}>
                                📍 {order.bakers.pickup_address}
                              </a>
                            ) : (
                    ...
                    {/* Delivery card — shown when delivered */}
                      ...
                      <div className="mb-4 rounded-xl overflow-hidden border" style={{ borderColor: '#e0d5cc' }}>
                        ...
                        <div className="px-4 py-3 flex gap-2 flex-wrap border-t" style={{ borderColor: '#e0d5cc' }}>
                          {submittedReviews.includes(order.id) && (
                            <span className="px-4 py-2 rounded-lg text-xs font-semibold" style={{ backgroundColor: '#dcfce7', color: '#166534' }}>✓ Review submitted</span>
                          )}
                          <button onClick={() => {
                            const text = 'Just got my order from ' + order.bakers?.business_name + ' via @Whiskly 🎂 So beautiful!'
                            if (navigator.share) {
                              navigator.share({ text, url: 'https://whiskly.vercel.app/bakers/' + order.baker_id })
                            } else {
                              navigator.clipboard.writeText(text + ' https://whiskly.vercel.app/bakers/' + order.baker_id)
                    ...
                    {/* Pickup handoff card — show confirm pickup when handoff photo uploaded */}
                    ...
                    {/* Complete card for pickup orders — nudge handles review prompt */}
                    {order.status === 'complete' && order.pickup_confirmed_at && !order.delivery_proof_url && (
                      <div className="mb-4 rounded-xl p-4" style={{ backgroundColor: '#f5f0eb' }}>
                        ...
                        {submittedReviews.includes(order.id) && (
                          <p className="text-xs mt-2 font-semibold" style={{ color: '#166534' }}>✓ Review submitted — thank you!</p>
                        )}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-2 md:gap-4 py-4 border-t border-b mb-4" style={{ borderColor: '#e0d5cc' }}>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium" style={{ color: '#2d1a0e' }}>{(() => { const [y,m,d] = order.event_date.split('-').map(Number); return new Date(y,m-1,d).toLocaleDateString([], {month:'long',day:'numeric',year:'numeric'}) })()}</p>
                      </div>
                      <div>
                    ...
                    {/* Address info — shown after accepted */}
                    {(order.status === 'confirmed' || order.status === 'in_progress' || order.status === 'ready' || order.status === 'complete') && order.fulfillment_type === 'pickup' && (
                      <div className="mb-4 px-3 py-2.5 rounded-xl text-xs" style={{ backgroundColor: '#f5f0eb' }}>
                        ...
                        {order.status !== 'ready' && order.status !== 'complete' && (
                          <span className="ml-1 italic" style={{ color: '#8B4513' }}> · Full address shared when ready</span>
                        )}
                      </div>
                    )}
                    ...
                    {/* Baker question — prominent, actionable */}
                    {order.baker_question && (
                      <div className="mb-4 rounded-xl border-2 p-4 flex items-start justify-between gap-4"
                        style={{ borderColor: '#f59e0b', backgroundColor: '#fffbeb' }}>
                        <div className="flex items-start gap-3">
                          <span className="text-lg flex-shrink-0">💬</span>
                          <div>
                            <p className="text-xs font-bold mb-1" style={{ color: '#92400e' }}>
                              {order.bakers?.business_name} has a question about your order:
                            </p>
                    ...
                    {/* 48hr delayed review nudge — appears only 48+ hrs after completion */}
                    {reviewNudgeEligible(order) && (
                      <div className="mb-4 rounded-xl px-4 py-3 flex items-center justify-between gap-3"
                        ...
                        <button
                          ...
                          className="flex-shrink-0 px-4 py-2 rounded-lg text-white text-xs font-semibold"
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button onClick={() => openThread(order.id)}
                        className="px-4 py-2 rounded-lg text-xs font-semibold border"
                      ...
                      {order.status === 'declined' && (
                        <Link href="/bakers" className="px-4 py-2 rounded-lg text-xs font-semibold text-white" style={{ backgroundColor: '#2d1a0e' }}>
                          Find Another Baker
                        </Link>
                      )}
                    </div>
                ...
                <div className="flex-1 overflow-y-auto">
                  {conversations.length === 0 ? (
                    ...
                    conversations.map(conv => (
                      <button key={conv.thread_key}
                        onClick={() => { setActiveOrderId(conv.order_id); loadThread(conv.order_id) }}
                        className="w-full px-4 py-3 flex items-center gap-3 text-left border-b"
                        style={{
                          borderColor: '#e0d5cc',
                          backgroundColor: activeOrderId === conv.order_id ? '#f5f0eb' : 'transparent',
                          borderLeft: activeOrderId === conv.order_id ? '3px solid #2d1a0e' : '3px solid transparent'
                        }}>
                        <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: '#f5f0eb' }}>
                          {conv.bakers?.profile_photo_url
                            ? <img src={conv.bakers.profile_photo_url} alt="" className="w-full h-full object-cover" />
                            : <span className="text-lg">🎂</span>}
                        </div>
                ...
                {!activeOrderId || !activeConvo ? (
                  ...
                  <>
                    ...
                    <div className="px-5 py-4 border-b flex items-center gap-3" style={{ borderColor: '#e0d5cc' }}>
                      ...
                      <button className="md:hidden flex-shrink-0 text-sm font-semibold mr-1"
                      ...
                      <div className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: '#f5f0eb' }}>
                        {activeConvo.baker?.profile_photo_url
                          ? <img src={activeConvo.baker.profile_photo_url} alt="" className="w-full h-full object-cover" />
                          : <span>🎂</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs" style={{ color: '#8B4513' }}>
                          {activeConvo.order?.event_type}{activeConvo.order?.event_date ? ' · ' + (() => { const [y,m,d] = activeConvo.order.event_date.split('-').map(Number); return new Date(y,m-1,d).toLocaleDateString([], {month:'short',day:'numeric',year:'numeric'}) })() : ''}
                        </p>
                      </div>
                      <Link href={'/bakers/' + activeConvo.baker?.id}
                            ...
                            <div key={msg.id}>
                              {showDate && (
                                <div className="text-center my-2">
                                  <span className="text-xs px-3 py-1 rounded-full" style={{ backgroundColor: '#f5f0eb', color: '#5c3d2e' }}>{formatDate(msg.created_at)}</span>
                                </div>
                              )}
                    ...
                    <div className="px-5 py-4 border-t flex gap-3 items-end" style={{ borderColor: '#e0d5cc' }}>
                      ...
                      <button onClick={sendMessage} disabled={sendingMessage || !newMessage.trim()}
                        ...
                        style={{ backgroundColor: '#2d1a0e', opacity: (!newMessage.trim() || sendingMessage) ? 0.5 : 1 }}>
                        Send
                      </button>
                    </div>
                  </>
        ...
        {activeTab === 'nearby' && (
          <div className="grid grid-cols-2 gap-4">
            {nearbyBakers.length === 0 ? (
              <div className="col-span-1 sm:col-span-2 bg-white rounded-2xl p-16 shadow-sm text-center">
                <p className="text-5xl mb-4">📍</p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="px-4 py-3 rounded-lg border text-sm" style={{ borderColor: '#e0d5cc', color: '#2d1a0e', backgroundColor: '#faf8f6' }}>{customer?.full_name || '—'}</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1" style={{ color: '#2d1a0e' }}>Email</label>
                  <p className="px-4 py-3 rounded-lg border text-sm" style={{ borderColor: '#e0d5cc', color: '#2d1a0e', backgroundColor: '#faf8f6' }}>{customer?.email || '—'}</p>
                </div>
                <div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              <div className="mb-5">
                ...
                <p className="text-xs mt-1" style={{ color: '#5c3d2e' }}>
                  Save recurring dates and we'll remind you to order in advance — birthdays, anniversaries, and more.
                </p>
              </div>
              ...
              {importantDates.length > 0 && (
                <div className="flex flex-col gap-2 mb-5">
                  {importantDates.map(date => {
                    ...
                    const soon = daysAway <= 42 // within 6 weeks
                    return (
                      <div key={date.id} className="flex items-center justify-between px-4 py-3 rounded-xl"
                        style={{ backgroundColor: soon ? '#fff7ed' : '#f5f0eb' }}>
                        <div className="flex items-center gap-3">
                          <div className="text-center w-10 flex-shrink-0">
                            <p className="text-xs font-bold uppercase" style={{ color: soon ? '#c2410c' : '#5c3d2e' }}>
                              {MONTHS[date.month - 1].slice(0, 3)}
                            </p>
                            <p className="text-lg font-bold leading-none" style={{ color: soon ? '#c2410c' : '#2d1a0e' }}>
                              {date.day}
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="text-xs mt-0.5" style={{ color: soon ? '#c2410c' : '#5c3d2e' }}>
                              ...
                              {soon && daysAway > 1 ? ' — time to order!' : ''}
                            </p>
                          </div>
                        </div>
                        <button onClick={() => removeImportantDate(date.id)}
                          className="text-xs px-2 py-1 rounded-lg border flex-shrink-0"
                          style={{ borderColor: '#e0d5cc', color: '#5c3d2e' }}>
                          ✕
                        </button>
                      </div>
              ...
              <div className="p-4 rounded-xl border-2 border-dashed" style={{ borderColor: '#e0d5cc' }}>
                ...
                <div className="flex flex-col gap-2">
                  <input
                    ...
                    placeholder="e.g. Emma's Birthday, Our Anniversary..."
                    className="w-full px-3 py-2.5 rounded-lg border text-sm"
                    style={{ borderColor: '#e0d5cc', color: '#2d1a0e', backgroundColor: '#faf8f6' }} />
                  <div className="flex gap-2">
                    <select value={newDateMonth} onChange={e => setNewDateMonth(e.target.value)}
                      className="flex-1 px-3 py-2.5 rounded-lg border text-sm"
                      style={{ borderColor: '#e0d5cc', color: newDateMonth ? '#2d1a0e' : '#5c3d2e', backgroundColor: '#faf8f6' }}>
                      <option value="">Month</option>
                      {MONTHS.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                    </select>
                    <select value={newDateDay} onChange={e => setNewDateDay(e.target.value)}
                      className="w-24 px-3 py-2.5 rounded-lg border text-sm"
                      style={{ borderColor: '#e0d5cc', color: newDateDay ? '#2d1a0e' : '#5c3d2e', backgroundColor: '#faf8f6' }}>
                    ...
                    <button
                      ...
                      disabled={savingDate || !newDateLabel.trim() || !newDateMonth || !newDateDay}
              ...
              {importantDates.length === 0 && (
                <p className="text-xs text-center mt-4" style={{ color: '#5c3d2e' }}>
                  No dates saved yet — add your first one above!
                </p>
              )}
            </div>
