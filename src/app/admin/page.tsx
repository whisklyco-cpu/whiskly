    ...
    <div className="min-h-screen" style={{ backgroundColor: '#f5f0eb' }}>
      <nav className="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-xs" style={{ color: '#5c3d2e' }}>
          {disputedOrders.length > 0 && <span className="px-2 py-1 rounded-full font-semibold" style={{ backgroundColor: '#fee2e2', color: '#991b1b' }}>{disputedOrders.length} dispute{disputedOrders.length > 1 ? 's' : ''}</span>}
          {pendingApplications.length > 0 && <span className="px-2 py-1 rounded-full font-semibold" style={{ backgroundColor: '#fef9c3', color: '#854d0e' }}>{pendingApplications.length} application{pendingApplications.length > 1 ? 's' : ''}</span>}
        ...
        {activeTab === 'Overview' && (
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Total GMV', value: '$' + totalGMV.toFixed(2), sub: 'all paid orders' },
                { label: 'Commission Earned', value: '$' + totalCommission.toFixed(2), sub: '10% / 7% Pro' },
                { label: 'Pro Bakers', value: proCount, sub: 'of ' + bakers.length + ' total' },
                { label: 'Pending Orders', value: pendingOrders, sub: 'awaiting baker' },
              ].map(stat => (
                <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: '#9c7b6b' }}>{stat.sub}</p>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <p className="text-sm font-bold mb-3" style={{ color: '#2d1a0e' }}>Platform Stats</p>
                {[
                  ['Total Bakers', bakers.length],
                  ['Active Bakers', bakers.filter(b => b.is_active).length],
                  ['Total Customers', customers.length],
                  ['Total Orders', orders.length],
                  ['Completed Orders', orders.filter(o => o.status === 'complete').length],
                  ['Disputed Orders', disputedOrders.length],
                ].map(([label, val]) => (
                  <div key={label as string} className="flex justify-between py-2 border-b last:border-b-0" style={{ borderColor: '#f5f0eb' }}>
                    <span className="text-xs" style={{ color: '#5c3d2e' }}>{label}</span>
                    <span className="text-xs font-bold" style={{ color: '#2d1a0e' }}>{val}</span>
                  </div>
                ))}
              </div>

              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <p className="text-sm font-bold mb-3" style={{ color: '#2d1a0e' }}>Recent Orders</p>
                {orders.slice(0, 6).map(o => (
                  <div key={o.id} className="flex justify-between py-2 border-b last:border-b-0" style={{ borderColor: '#f5f0eb' }}>
                    <div>
                      <p className="text-xs font-semibold" style={{ color: '#2d1a0e' }}>{o.customer_name}</p>
                      <p className="text-xs" style={{ color: '#5c3d2e' }}>{o.event_type} · {o.bakers?.business_name}</p>
                    </div>
                    <span className="text-xs px-2 py-0.5 rounded-full h-fit" style={{
                      backgroundColor: o.status === 'complete' ? '#dcfce7' : o.status === 'pending' ? '#fef9c3' : '#dbeafe',
                      color: o.status === 'complete' ? '#166534' : o.status === 'pending' ? '#854d0e' : '#1e40af'
                    }}>{o.status}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                <p className="text-sm font-bold mb-3" style={{ color: '#2d1a0e' }}>Needs Attention</p>
                {disputedOrders.length === 0 && pendingApplications.length === 0 ? (
                  <p className="text-xs" style={{ color: '#5c3d2e' }}>All clear!</p>
                ) : (
                  <>
                    {disputedOrders.slice(0, 3).map(o => (
                      <div key={o.id} className="py-2 border-b" style={{ borderColor: '#f5f0eb' }}>
                        <p className="text-xs font-semibold" style={{ color: '#991b1b' }}>Dispute — {o.customer_name}</p>
                        <p className="text-xs" style={{ color: '#5c3d2e' }}>{o.event_type} · ${o.budget}</p>
                      </div>
                    ))}
                    {pendingApplications.slice(0, 3).map(b => (
                      <div key={b.id} className="py-2 border-b last:border-b-0" style={{ borderColor: '#f5f0eb' }}>
                        <p className="text-xs font-semibold" style={{ color: '#854d0e' }}>Application — {b.business_name}</p>
                        <p className="text-xs" style={{ color: '#5c3d2e' }}>{b.city}, {b.state}</p>
                      </div>
                    ))}
                  </>
                )}
                    ...
                    .map(order => (
                      <tr key={order.id} className="border-b" style={{ borderColor: '#f5f0eb', backgroundColor: order.is_flagged ? '#fff7ed' : order.is_disputed ? '#fef2f2' : 'transparent' }}>
                        ...
                        <td className="py-2.5 px-3">
                          <span style={{ color: order.deposit_paid_at ? '#166534' : '#854d0e' }}>
                            {order.deposit_paid_at ? '✓ Paid' : 'Unpaid'}
                          </span>
                        </td>
                        <td className="py-2.5 px-3">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {bakers
                    .filter(b => !bakerSearch || b.business_name?.toLowerCase().includes(bakerSearch.toLowerCase()) || b.email?.toLowerCase().includes(bakerSearch.toLowerCase()))
                    .map(baker => {
                      ...
                      return (
                        <tr key={baker.id} className="border-b" style={{ borderColor: '#f5f0eb', backgroundColor: baker.is_suspended ? '#fef2f2' : 'transparent' }}>
                          ...
                          <td className="py-2.5 px-3">
                            <span style={{ color: baker.stripe_account_id ? '#166534' : '#854d0e' }}>
                              {baker.stripe_account_id ? '✓ Connected' : 'Not connected'}
                            </span>
                          </td>
                          <td className="py-2.5 px-3">
                            <span className="px-2 py-0.5 rounded-full" style={{
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {customers
                    .filter(c => !customerSearch || c.full_name?.toLowerCase().includes(customerSearch.toLowerCase()) || c.email?.toLowerCase().includes(customerSearch.toLowerCase()))
                    .map(customer => {
                      ...
                      return (
                        <tr key={customer.id} className="border-b" style={{ borderColor: '#f5f0eb', backgroundColor: customer.is_suspended ? '#fef2f2' : 'transparent' }}>
                          ...
                          <td className="py-2.5 px-3">
                            <span className="px-2 py-0.5 rounded-full" style={{
                              backgroundColor: customer.is_suspended ? '#fee2e2' : '#dcfce7',
                              color: customer.is_suspended ? '#991b1b' : '#166534'
                            }}>
                          ...
                          <td className="py-2.5 px-3">
                            <button onClick={() => toggleCustomerSuspend(customer)}
                              className="px-2 py-1 rounded text-xs font-semibold border"
                              style={{ borderColor: customer.is_suspended ? '#166534' : '#dc2626', color: customer.is_suspended ? '#166534' : '#dc2626' }}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        {baker.specialties?.length > 0 && (
                          <p className="text-xs mt-1" style={{ color: '#8B4513' }}>{baker.specialties.join(', ')}</p>
                        )}
